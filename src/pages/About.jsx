...
const About = () => {
  return (
    <div className="min-h-screen bg-[var(--surface)] dark:bg-[var(--surface-dark)]">
      <section className="bg-gradient-to-br from-[var(--sand-almond-silk)] to-[var(--sand-accent)] text-white py-24">
        ...
        <div className="container-sand">
          <div className="grid md:grid-cols-2 gap-12 items-center">
            <motion.div initial={{ opacity: 0, x: -50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }}>
              ...
              <h2 className="text-4xl font-bold text-[var(--text-ink)] dark:text-[var(--text-dark-ink)] mb-6 mt-4">
                শিক্ষা হওয়া উচিত সহজ, আকর্ষণীয় ও রূপান্তরকারী
              </h2>
              <p className="text-lg text-[var(--text-soft)] dark:text-[var(--text-dark-soft)] mb-4">
            ...
            <motion.div initial={{ opacity: 0, x: 50 }} whileInView={{ opacity: 1, x: 0 }} viewport={{ once: true }} className="relative">
              <div className="bg-gradient-to-br from-[var(--sand-linen)] to-[var(--sand-almond-cream)] dark:from-[var(--surface-dark-soft)] dark:to-[var(--surface-dark-card)] rounded-3xl p-12">
                <Target className="w-24 h-24 text-[var(--sand-accent-strong)] dark:text-[var(--sand-almond-silk)] mx-auto" />
              </div>
            </motion.div>
          </div>
        </div>
      ...
      <section className="py-24 bg-[var(--surface-soft)] dark:bg-[var(--surface-dark-soft)]">
        <div className="container-sand">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} className="text-center mb-12">
            ...
            <h2 className="text-4xl font-bold text-[var(--text-ink)] dark:text-[var(--text-dark-ink)] mb-4 mt-4">যা আমাদের চালায়</h2>
            <p className="text-lg text-[var(--text-soft)] dark:text-[var(--text-dark-soft)]">যে নীতিগুলো আমাদের সব কিছুর ভিত্তি</p>
          </motion.div>
          <div className="grid md:grid-cols-3 gap-8">
            {[
              ...
              <motion.div key={index} initial={{ opacity: 0, y: 50 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="sand-card p-8 text-center">
                <div className="w-20 h-20 bg-[var(--sand-linen)] dark:bg-[var(--surface-dark-soft)] rounded-2xl flex items-center justify-center text-[var(--sand-accent-strong)] dark:text-[var(--sand-almond-silk)] mx-auto mb-6">
                  {value.icon}
                </div>
                <h3 className="text-xl font-bold text-[var(--text-ink)] dark:text-[var(--text-dark-ink)] mb-3">{value.title}</h3>
                <p className="text-[var(--text-soft)] dark:text-[var(--text-dark-soft)]">{value.description}</p>
              </motion.div>
            ))}
          </div>
        </div>
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-8">
            {[
              ...
              <motion.div key={index} initial={{ opacity: 0, scale: 0.8 }} whileInView={{ opacity: 1, scale: 1 }} viewport={{ once: true }} transition={{ delay: index * 0.1 }} className="text-center">
                <div className="text-[var(--sand-accent-strong)] dark:text-[var(--sand-almond-silk)] mb-4 flex justify-center">{stat.icon}</div>
                <div className="text-4xl font-bold text-[var(--text-ink)] dark:text-[var(--text-dark-ink)] mb-2">{stat.number}</div>
                <div className="text-[var(--text-soft)] dark:text-[var(--text-dark-soft)] font-semibold">{stat.label}</div>
              </motion.div>
            ))}
          </div>
        </div>
      ...
      <section className="py-24 bg-gradient-to-r from-[var(--sand-almond-silk)] to-[var(--sand-accent)]">
        <div className="container-sand text-center">
          <motion.div initial={{ opacity: 0, y: 20 }} whileInView={{ opacity: 1, y: 0 }} viewport={{ once: true }}>
            ...
            <Link to="/courses">
              <motion.button whileHover={{ scale: 1.05 }} whileTap={{ scale: 0.95 }} className="px-8 py-4 bg-white text-[var(--sand-accent-strong)] rounded-full font-bold text-lg shadow-xl hover:shadow-2xl transition-all">
